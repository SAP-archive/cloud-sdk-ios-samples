...
class SupplierMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([Supplier]?, Error?) -> Void) -> Void)?
    private var entities: [Supplier] = [Supplier]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let supplier = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ID", value: "\(supplier.id!)")
        return cell
    }
...
extension SupplierMasterViewController: EntitySetUpdaterDelegate {
